...
import com.example.standard.controller.ImageGet;
...
public class WebOCR {
	 ...
	 * @throws IOException
	 */
	public String imageSignTest() throws IOException {
	...
	@PostMapping("/getUrl")
	public String getUrl(MultipartFile file, Model model) throws IOException {
	String imageurl=file.getOriginalFilename();
	File file2 = new File("d:\\postgraduate\\"+imageurl);
	file.transferTo(file2);
	System.out.print(imageurl);
	this.PIC_PATH=file2.getPath();

	Boolean flag = imageSignTest().contains("Ex");
		String question="";
	if (!flag){
		 question = "防爆设备标识异常";
	}else {
		...
		model.addAttribute("path",file2.getPath());
		model.addAttribute("question",question);

		System.out.print(imageSignTest());
		return "/imgocr";
